...
def predict_image(image, model, signal):
    ...
    try:
        ...
        if not isinstance(pred,str):
            img_np = copy.deepcopy(np.array(img, dtype=np.uint8)) 
            draw_own_bbox(img_np, pred['xmin'], pred['ymin'], pred['xmax'], pred['ymax'], pred['name'])
            # draw_own_bbox(np.array(img, dtype=np.uint8), pred['xmin'], pred['ymin'], pred['xmax'], pred['ymax'], pred['name'])
    ...
    # except:
        ...
        print(f"Error processing image: {e}") # Print error to console
